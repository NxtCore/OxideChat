...
use super::{Chat, Message, Workspace};

impl Workspace {
	pub async fn clear_default_for_user(pool: &PgPool, user_id: &Uuid, exclude_id: Option<&Uuid>) -> Result<(), sqlx::Error> {
...
impl Message {
	pub async fn deactivate_subtree(tx: &mut sqlx::Transaction<'_, sqlx::Postgres>, message_id: &Uuid) -> Result<(), sqlx::Error> {
		sqlx::query(
			r#"
	...
	pub async fn activate_subtree(tx: &mut sqlx::Transaction<'_, sqlx::Postgres>, message_id: &Uuid) -> Result<(), sqlx::Error> {
		sqlx::query(
			r#"
			WITH RECURSIVE descendants AS (
				SELECT id FROM messages WHERE id = $1
	...
	pub async fn switch_to_fork(pool: &PgPool, chat_id: &Uuid, parent_id: Option<&Uuid>, fork_index: i32) -> Result<Option<Self>, sqlx::Error> {
		...
		let target = sqlx::query_as::<_, Message>(
			"SELECT * FROM messages WHERE chat_id = $1 AND parent_id IS NOT DISTINCT FROM $2 AND fork_index = $3",
		)
		.bind(chat_id)
		.bind(parent_id)
		.bind(fork_index)
