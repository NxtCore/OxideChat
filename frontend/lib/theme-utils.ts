import type {ThemeCssVars, FetchedTheme} from '~/types/chat';

export const THEME_URLS = [
	'https://tweakcn.com/themes/cmk8z9ndk000204jw8g884fcy',
...
export function convertToThemeCssVars(externalTheme: any): ThemeCssVars {
...
export async function fetchThemeFromUrl(url: string): Promise<FetchedTheme> {
	...
	const transformedUrl =
		url.replace('https://tweakcn.com/editor/theme?theme=', baseUrl).replace('https://tweakcn.com/themes/', baseUrl) + (isBuiltInUrl ? '.json' : '');
	...
	try {
		...
		const cssVars = convertToThemeCssVars(themeData);
...
export function extractThemeColors(cssVars: ThemeCssVars, mode: 'light' | 'dark'): string[] {
	const colors: string[] = [];
	const modeVars = mode === 'light' ? cssVars.light : cssVars.dark;
	const colorKeys = ['primary', 'accent', 'secondary', 'background', 'muted', 'destructive', 'border', 'card', 'popover'];
	const currentVars = {...cssVars.theme, ...modeVars};

	colorKeys.forEach(key => {
		const colorValue = currentVars[key];
		if (colorValue && colors.length < 5) {
			if (colorValue.includes('hsl') || colorValue.includes('oklch')) {
